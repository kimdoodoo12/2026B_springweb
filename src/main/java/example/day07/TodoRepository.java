...
public interface TodoRepository extends JpaRepository<TodoEntity, Integer>{
    // extends JpaRepository<조작할 엔티티명 , PK타입명>
    // 1. CRUD 메소드 제공: .save() .findAll() .findById() .deleteById() 등
    // 2. 쿼리메소드 : SQL 작성하지 않고 추상메소드 이름으로 쿼리 자동생성<카멜표기법>
    // 반환타입 findBy필드명(타입 매개변수명); 주의할점: 필드명에 대소문자(카멜) 규칙 , 존재하는 필드명 기능
    ...
    // 추상메소드 위에 @Query(value = "직접 SQL", nativeQuery = true)
    // SQL 문법 내 매개변수 대입시 ? 대신에 :
